...
import { db, initSchema, seedAccounts } from './db.js';
    ...
    const result = await db.execute({
      sql: `
        SELECT m.id, m.channel, m.author, m.content, m.type,
               ...
               GROUP_CONCAT(CASE WHEN r.type='like'    THEN r.username END) AS likes,
               GROUP_CONCAT(CASE WHEN r.type='dislike' THEN r.username END) AS dislikes
        FROM   messages  m
